...
send_mail()
{
	...
	if [ $1 = 0 ]
	then
		echo -e $success_msg | mailx -s "[NCDEX] TMP FILE STATUS FOR USER $user - OK" $MAIL_RECP
	else
		echo -e $fail_msg | mailx -s "[NCDEX] TMP FILE STATUS FOR USER $user - ERROR" $MAIL_RECP
	fi	
}
